...
The `home-routes` handler in the `steam-filter.routes.home` namespace
...
The `home-page` function will in turn call the `steam-filter.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/steam_filter/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `steam-filter.handler` namespace:
...
Request middleware functions are located under the `steam-filter.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `steam-filter.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `steam-filter.core` in some other namespace.
